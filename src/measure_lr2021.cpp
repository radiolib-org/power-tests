...
const int pwr_min = -18;
const int pwr_max = 44;
const int paDutyCycle_min = 1;
const int paDutyCycle_max = 7;
const int paSlices_min = 0;
const int paSlices_max = 7;

PiHal* hal = new PiHal(0);
LR2021 radio = new RadioModuleHal(hal);
...
int main(int argc, char** argv) {
  ...
  // LR2021 cannot set overcurrent limit
  // print this to not mess up the log format
  fprintf(stdout, "setCurrentLimit skipped\n\n");

  // print the header
  fprintf(stdout, "Set [dBm/2],paDutyCycle,paSlc,Measured [dBm],Bus voltage[V],Voltage[mV],Current[mA],Power [mW],Efficiency [%%]\n"); 
  ...
  for(int pwr = pwr_min; pwr <= pwr_max; pwr++) {
    for(int paDutyCycle = paDutyCycle_min; paDutyCycle <= paDutyCycle_max; paDutyCycle++) {
      for(int paSlices = paSlices_min; paSlices <= paSlices_max; paSlices++) {
        // set the configuration and start transmitting
        radio.setOutputPower(pwr, 48, paDutyCycle, paSlices);
        ...
        dc_powermon_read_power(&p_shunt);
          ...
          // print it out
          fprintf(stdout, "%11d,%11d,%5d,%14.2f,%14.2f,%11.2f,%11.2f,%10.2f,%14.2f\n",
            pwr, paDutyCycle, paSlices, (double)rf_pwr, (double)v_bus,
